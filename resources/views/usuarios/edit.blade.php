...
<div class="d-flex justify-content-center align-items-center" style="min-height: 80vh;">
    <div class="card shadow-sm rounded-lg" style="max-width: 600px; width: 100%;">
        ...
        <div class="card-body p-4">
            <form method="POST" action="{{ route('usuarios.update', $v_usuario->rut) }}">
                @csrf
                @method('put')
                
                <div class="mb-3">
                    <label for="rut" class="form-label">Rut</label>
                    <input type="text" class="form-control" id="rut" name="rut" value="{{ $v_usuario->rut }}" placeholder="Rut"readonly>
                    @error('rut')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nombre" name="nombre" value="{{ $v_usuario->nombre }}" placeholder="Nombre del Usuario">
                    @error('nombre')
                    <div class="text-danger small">{{ $message }}</div>
                    @enderror 
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="apellido" name="apellido" value="{{ $v_usuario->apellido }}" placeholder="Apellido">
                    @error('apellido')
                    <div class="text-danger small">{{ $message }}</div>
                    @enderror 
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('rol') is-invalid @enderror" id="rol" name="rol">
                        ...
                        @foreach ($roles as $rol)
                            <option value="{{ $rol->id_rol }}" {{ $v_usuario->id_rol == $rol->id_rol ? 'selected' : '' }}>
                                {{ $rol->nombre_rol }}
                            </option>
                        @endforeach
                    </select>
